...
class ApiService {
  ...
  Future<bool> registrarAsignatura(String codigo, String nombre, int creditos, int idPrograma)async{
    final token = await _storage.read(key: 'token');
    dio.options.headers['Authorization'] = 'Bearer $token';

    try {
      final response = await dio.post('/admin/registrar/asignatura',
      data: {
        "codigo":codigo,
        "nombre": nombre,
        "creditos": creditos,
  ...
  Future<bool> crearPrograma(String nombre, String descripcion)async{
    ...
    try {
      final response = await dio.post('/admin/registrar/programa',
      data: {
        "nombre": nombre,
  ...
  Future<bool> crearSemestre(String fechaInicio, String fechaFin, String nombre)async{
    final token = await _storage.read(key: 'token');
    dio.options.headers['Authorization'] = 'Bearer $token';
  ...
  Future<bool> habilitarAsignatura(int idAsignatura)async{
    ...
    dio.options.headers['Authorization'] = 'Bearer $token';
    try{
      final response = await dio.put('/admin/avilitar/asignatura/$idAsignatura');
